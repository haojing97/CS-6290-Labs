...
bool rob_check_space(ROB *rob)
{
    ...
    return ((rob->tail_ptr + 1) % (NUM_ROB_ENTRIES + 1)) != rob->head_ptr;
}
...
int rob_insert(ROB *rob, InstInfo inst)
{
    ...
    if (rob_check_space(rob))
    {
        ...
        rob->tail_ptr = (rob->tail_ptr + 1) % (NUM_ROB_ENTRIES + 1);
        return index;
    }
    return -1;
}
...
void rob_wakeup(ROB *rob, int tag)
{
    ...
    for (unsigned int i = 0; i < NUM_ROB_ENTRIES + 1; i++)
    {
...
InstInfo rob_remove_head(ROB *rob)
{
    ...
    if (rob_check_head(rob))
    {
        ...
        inst = &rob->entries[rob->head_ptr].inst;
        rob->head_ptr = (rob->head_ptr + 1) % (NUM_ROB_ENTRIES + 1);
        return *inst;
    }
}
