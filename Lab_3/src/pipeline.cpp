...
void pipe_cycle_issue(Pipeline *p)
{
    ...
    for (unsigned int i = 0; i < PIPE_WIDTH; i++)
    {
        ...
        for (unsigned int j = 0; j < PIPE_WIDTH; j++)
        {
            if (p->ID_latch[j].valid && idx == -1)
            {
                idx = j;
            }
            else if (p->ID_latch[j].valid && (p->ID_latch[j].inst.inst_num < p->ID_latch[idx].inst.inst_num))
            {
                idx = j;
            }
        }
        ...
        int tag = rob_insert(p->rob, p->ID_latch[idx].inst);
        if (tag == -1)
        {
            continue;
        }
        p->ID_latch[idx].valid = false;
        InstInfo *inst = &p->rob->entries[tag].inst;
        int prf_id;
        prf_id = rat_get_remap(p->rat, inst->src1_reg);
        if (prf_id == -1)
        {
        ...
        {
            inst->src1_tag = prf_id;
            inst->src1_ready = p->rob->entries[prf_id].ready;
        }
        prf_id = rat_get_remap(p->rat, inst->src2_reg);
        if (prf_id == -1)
        {
        ...
        {
            inst->src2_tag = prf_id;
            inst->src2_ready = p->rob->entries[prf_id].ready;
        }
        inst->dr_tag = tag;
        if (inst->dest_reg != -1)
        {
            rat_set_remap(p->rat, inst->dest_reg, tag);
        }
    }
}
...
void pipe_cycle_schedule(Pipeline *p)
{
    ...
    if (SCHED_POLICY == SCHED_IN_ORDER)
    {
        ...
        for (unsigned int i = 0; i < PIPE_WIDTH; i++)
        {
            int index = p->rob->head_ptr;
            while (index != p->rob->tail_ptr)
            {
                if (p->rob->entries[index].valid && !p->rob->entries[index].exec)
                {
                    if (p->rob->entries[index].inst.src1_ready && p->rob->entries[index].inst.src2_ready)
                    {
                        rob_mark_exec(p->rob, p->rob->entries[index].inst);
                        PipelineLatch sc;
                        sc.inst = p->rob->entries[index].inst;
                        sc.valid = true;
                        sc.stall = false;
                        p->SC_latch[i] = sc;
                    }
                ...
                index = (index + 1) % (NUM_ROB_ENTRIES + 1);
            }
        }
    }
    ...
    if (SCHED_POLICY == SCHED_OUT_OF_ORDER)
    {
        ...
        for (unsigned int i = 0; i < PIPE_WIDTH; i++)
        {
            ...
            while (index != p->rob->tail_ptr)
            {
                if (p->rob->entries[index].valid &&
                    p->rob->entries[index].inst.src1_ready && p->rob->entries[index].inst.src2_ready &&
                    !p->rob->entries[index].exec)
                {
                    rob_mark_exec(p->rob, p->rob->entries[index].inst);
                    PipelineLatch sc;
                    sc.inst = p->rob->entries[index].inst;
...
void pipe_cycle_commit(Pipeline *p)
{
    ...
    for (unsigned int i = 0; i < PIPE_WIDTH; i++)
    {
        if (rob_check_head(p->rob))
        {
            ...
            if (p->rat->entries[inst.dest_reg].prf_id == inst.dr_tag)
            {
