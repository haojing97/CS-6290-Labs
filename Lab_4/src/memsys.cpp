                              ...
                              AccessType type, unsigned int core_id)
{
    ...
    if (type == ACCESS_TYPE_IFETCH)
    {
        ...
        outcome = cache_access(sys->icache, line_addr, false, core_id);
        if (outcome == MISS)
        {
    ...
    if (type == ACCESS_TYPE_LOAD)
    {
        ...
        outcome = cache_access(sys->dcache, line_addr, false, core_id);
        if (outcome == MISS)
        {
            delay += memsys_l2_access(sys, line_addr, false, core_id);
    ...
    if (type == ACCESS_TYPE_STORE)
    {
        ...
        delay += DCACHE_HIT_LATENCY;
        outcome = cache_access(sys->dcache, line_addr, true, core_id);
        if (outcome == MISS)
        {
            delay += memsys_l2_access(sys, line_addr, false, core_id);
    ...
    if (write_back)
    {
        uint64_t write_back_addr = (sys->dcache->last_evicted_line.tag * sys->dcache->num_sets) |
                                   (line_addr & (sys->dcache->num_sets - 1));
        memsys_l2_access(sys, write_back_addr, true, sys->dcache->last_evicted_line.core_id);
    }
                          ...
                          bool is_writeback, unsigned int core_id)
{
    ...
    if (outcome == MISS)
    {
        ...
        {
            uint64_t write_back_addr = (sys->l2cache->last_evicted_line.tag * sys->l2cache->num_sets) |
                                       (line_addr & (sys->l2cache->num_sets - 1));
            dram_access(sys->dram, write_back_addr, true);
        }
    }
                               ...
                               AccessType type, unsigned int core_id)
{
    ...
    Cache *c;
    CacheResult outcome;
    v_line_addr *= CACHE_LINESIZE;
    uint64_t vpn = v_line_addr / PAGE_SIZE;
    uint64_t pfn = memsys_convert_vpn_to_pfn(sys, vpn, core_id);
    uint64_t offset = v_line_addr & (PAGE_SIZE - 1);
    p_line_addr = (pfn * PAGE_SIZE) | offset;
    p_line_addr /= CACHE_LINESIZE;
    ...
    if (outcome == MISS)
    {
        ...
        {
            uint64_t write_back_addr = (c->last_evicted_line.tag * c->num_sets) |
                                       (p_line_addr & (c->num_sets - 1));
            memsys_l2_access(sys, write_back_addr, true, c->last_evicted_line.core_id);
        }
    }
